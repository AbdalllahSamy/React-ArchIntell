...
const CardDetails = () => {
    ...
    useEffect(() => {
        const fetchIsFollow = async () => {
            ...
            const query = `
              query isFollow($userToFollowId: ID!) {
                isFollow(userToFollowId: $userToFollowId)
              }
            `;
            const variables = {
            ...
            try {
                ...
                setFollow(response.data.data.isFollow)
            } catch (error) {
                console.error("Error fetching isFollow status:", error);
            }
        };

        if (post && post.creator && post.creator._id) {
            fetchIsFollow();
        }
    }, [post])
    ...
    const handleFollow = async () => {
        ...
        try {
            ...
            console.log(response);
            if (response.data) {
                setFollow(true) // Toggle follow state
            }
        } catch (error) {
